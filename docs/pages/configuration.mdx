...
<Tabs items={['Config with disabled theme', 'Config with enabled theme']}>
  <Tab>
...
function getColorAndEffectKeyName(name = '') {
...
module.exports = {
  ...
  styles: {
    exportPath: './ui/theme',
    allowedThemes: ['example-theme-1','example-theme-2'], // allowed themes
    // defaultTheme: 'example-theme-1', // If there is a default theme
    colors: {
      keyName: getColorAndEffectKeyName,
      useTheme: true,
    },
    effects: {
      keyName: getColorAndEffectKeyName,
...
* `allowedThemes` - list of allowed themes. By only these themes, it will be generated particular files.
* `defaultTheme` - one of allowed themes. Default variables for CSS variables will be taken from this theme.
...
module.exports = {
    "text-txt600":"var(--sh-text-txt600,'#ff0000')", // '#ff0000' - will be taken from a default theme if it is defined for a specific color
